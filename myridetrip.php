<?php
  ...
  $result1 = mysqli_query($conn,"SELECT * FROM ridetrip r, pendingrequest p, requeststatus s WHERE r.TID = p.TID AND p.RequestStatu = s.RequestStatus AND MID = $MID ORDER BY r.TID DESC");
  $result2 = mysqli_query($conn,"SELECT * FROM ridetrip t, pendingrequest p,rideshare s, requeststatus r WHERE t.TID = p.TID AND p.SID = s.SID AND p.RequestStatu = r.RequestStatus AND t.MID = $MID");
...
<body>
  ...
  -->
   ...
   <div class="container">
    <div id="main">
      <h3>My Ride Trip : Member ID/Name : <?php echo $row0['MID'] ?> / <?php echo $row0['memberName'] ?> </h3>
      <?php
      ...
      <table id="table1">
        ...
        <tr>
          ...
          <td> <?php echo $row1['NoOfPassengers'] ?></td>
          <td> <?php echo $row1['RemarksToDriver'] ?></td>
        </tr>
        <?php
        $i++;
        }
      ...
      <input type="button" class="button" value="Search For a Ride" onclick="GetSelect1()"/>
        <input type="button" class="button" value="View Accepted Ride Detail" onclick="GetSelect2()"/>
    </div>

    <!-- List of Pending Request Reply of a select RTrip -->
    <div id="main">
      ...
      <table id="table1">
        ...
        <tr>
          ...
          <td> <?php echo $row2['FarePerSeat'] ?></td>
          <td> <?php echo $row2['RemarksToPassenger'] ?> </td>
        </tr>
        <?php
        $i++;
        }
      ...
      <input type="button" class="button" value="Accept a Pending from Driver" onclick="GetSelect4()"/>
      <input type="button" class="button" value="Reject a Pending from Driver" onclick="GetSelect5()"/>
    </div>
