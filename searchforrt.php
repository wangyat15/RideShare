<?php
  ...
  if (!empty($_POST['sStartLocation'])) {
    ...
    $result = mysqli_query($conn,"SELECT * FROM ridetrip WHERE Date = '$sDate' AND Time = '$sTime' AND StartLocation = '$sStartLocation'");
  } else {           // seach for all rideshare records
      $sDate = date('Y-m-d');
      $sTime = date('H:i:s');
      $result = mysqli_query($conn,"SELECT * FROM ridetrip");
  }
?>
...
<body onload="initMap()">
  ...
  -->
   ...
   <div class="container">
    <div id="sideleft">
      ...
      <form method="post" onSubmit="return rtvalidate();" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
         <label><span id="lab">Ride Trip ID: </span><input type="text" /> </label></br>
        <!--      <form action="#" method="post"> -->
          ...
          <output>0</output>
          <br/>
          <span id=lab>Start location:</span> <input type="text" id=address name="sStartLocation" placeholder="Enter start location (w autocomplete)" style="width: 350px" maxlength="100"/> 
  ...
  <!--        <a href="index.html" class="button">Search </a></br> -->
          ...
          <a href="myrideshare.php" class="button">Return </a></br>
  ...
  <!--      </form> -->
    ...
    <div id="main">
      ...
      <table id="table1">
        ...
        <tr>
          <td><?php echo $i+1 ?></td>
          <td><?php echo '<a href="detailrt.php?tid='.$row['TID']. '">' .$row['TID']. '</a>' ?> </td>
          <td><?php echo '<a href="detailrt.php?tid='.$row['TID']. '">' .$row['Date']. '</a>' ?> </td>
          <td><?php echo '<a href="detailrt.php?tid='.$row['TID']. '">' .$row['Time']. '</a>' ?> </td>
          <td><?php echo '<a href="detailrt.php?tid='.$row['TID']. '">' .$row['StartLocation']. '</a>' ?> </td>
          <td><?php echo '<a href="detailrt.php?tid='.$row['TID']. '">' .$row['Destination']. '</a>' ?> </td>
          <td><?php echo '<a href="detailrt.php?tid='.$row['TID']. '">' .$row['Fare']. '</a>' ?> </td>
          <td><?php echo '<a href="detailrt.php?tid='.$row['TID']. '">' .$row['NoOfPassengers']. '</a>' ?> </td>
          <td><?php echo '<a href="detailrt.php?tid='.$row['TID']. '">' .$padded. '</a>' ?> </td>
        </tr>
        <?php
        $i++;
        }
